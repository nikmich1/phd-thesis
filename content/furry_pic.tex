\chapter{Bound state quantum electrodynamics in the Furry picture}
\label{ch:furry_pic}
In this thesis, relativistic atomic structure calculations are performed in order to investigate nuclear effects on the energy levels of electrons or muons bound in the potential of an atomic nucleus. In this chapter, the starting point is the Lagrangian of quantum electrodynamics (QED), and results will be derived from thereon. This includes the derivation of the Dirac equation for a classical field as the zero-order solution and radiative corrections due to virtual particles. The analytical solution of the Dirac equation for the relativistic Coulomb problem is discussed, as well as numerical solutions for arbitrary nuclear potentials. Finally, the leading order Vacuum polarization corrections are considered.
%\begin{itemize}
...
\begin{alignat}{2}
V_{\text{KS}}(r)=\frac{\alpha^2(Z\alpha)}{\pi r}\int\limits_0^\infty\text{d}r^\prime\,r^\prime \rho(r^\prime)\left( L_0(2m_e|r-r^\prime|)-L_0(2m_e(r+r^\prime))\right),
\end{alignat}
with
\begin{alignat}{2}
...
\end{equation}
Using this ansatz in Eq.~\eqref{eq:sphdirac} leads to the following system of equations for the radial functions $g_{n\kappa}(r)$ and $f_{n\kappa}(r)$:
\begin{alignat}{2}
