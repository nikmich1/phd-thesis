...
\clearpage
In conclusion, it was demonstrated that the bound-muon $g$ factor in helium-4 can be calculated on the $10^{-9}$ level. 

A measurement of this $g$ factor as $g_{\text{exp}}$ with a similar experimental uncertainty could give access to an independent determination of the muon mass, one order of magnitude more accurate than the current value. 
...
In the last row, the uncertainties due to the calculated and uncalculated (two-loop light-by-light) terms are given separately. The table and caption is taken from Ref.~\cite{sikora2018} and the contributions highlighted in red were performed in the framework of this thesis.
}
%\end{ruledtabular}
\end{footnotesize}
\end{table}
